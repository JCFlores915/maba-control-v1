    ...
    <Layout style={{ minHeight: '100vh' }}>
      <Sider collapsible>
        ...
        <Menu theme="dark" mode="inline" selectedKeys={[selectedKey]}>
          <Menu.Item key="1" icon={<DashboardOutlined />} onClick={() => navigate('/')}>Dashboard</Menu.Item>
          <Menu.Item key="2" icon={<SettingOutlined />} onClick={() => navigate('/settings')}>Configuración</Menu.Item>
        </Menu>
      </Sider>
      <Layout>
        <Header style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12 }}>
          ...
          <Button danger icon={<LogoutOutlined />} onClick={logout}>Salir</Button>
        </Header>
